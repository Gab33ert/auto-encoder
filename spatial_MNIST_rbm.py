...
def actualise_weight(visible, b, c, w, n, k, t):
    ...
    """
    if n%100==0:
        e=0#energy on test set
        E=0#training set
        hidden_test=sample_rbm_forward(x_test, c, w)
        for i in range(x_test.shape[1]):
            e-=(hidden_test[:,i]).dot(w.dot(x_test[:,i]))
        e+=-np.sum(b*x_test)-np.sum(c*hidden_test)
        
        hidden_train=sample_rbm_forward(x_train, c, w)
        for i in range(x_train.shape[1]):
            E-=(hidden_train[:,i]).dot(w.dot(x_train[:,i]))
        E+=-np.sum(b*x_train)-np.sum(c*hidden_train)
        return e/(x_test.shape[1]) #E/(x_train.shape[1])#, np.sum(np.sum(np.abs(visible-visible_c)))/(visible.shape[0]*visible.shape[1])
    """
