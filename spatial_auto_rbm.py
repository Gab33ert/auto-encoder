...
def sample_rbm_forward(visible, c, w, delta):
    return np.where(np.random.rand(w.shape[0],visible.shape[1]) < sigmoid(np.tile(c,(1,visible.shape[1]))+w.dot(visible)*(delta**(-2))), -1, 1)

def sample_grbm(hidden, b, w, delta):
    return np.random.normal(np.tile(b,(1,hidden.shape[1]))+np.transpose(w).dot(hidden),delta)#,(w.shape[1],hidden.shape[1]))

def actualise_weight(visible, b, c, w, delta):
    hidden=sample_rbm_forward(visible, c, w, delta)
    visible_c=sample_grbm(hidden, b, w, delta)
    hidden_c=sample_rbm_forward(visible, c, w, delta)
...
c=0.1*np.random.randn(wc.shape[0],1)
b=0.1*np.random.randn(wc.shape[1],1)
...
hidden=sample_rbm_forward(scaled_data, c, wc, delta)
sample_grbm(hidden, b, wc, delta)
actualise_weight(scaled_data, b, c, wc, delta)
